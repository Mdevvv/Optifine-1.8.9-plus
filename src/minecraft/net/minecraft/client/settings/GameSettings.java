...
public class GameSettings
{
    ...
    {
        this.setOptionValueOF(option, value);
    ...
    public String getKeyBinding(GameSettings.Options option)
    {
        ...
        {
            String optionName = I18n.format(option.getEnumString(), new Object[0]) + ": ";

            if (option.getEnumFloat())
            {
                float value = this.getOptionFloatValue(option);
                float normalisedValue = option.normalizeValue(value);
                return option == GameSettings.Options.MIPMAP_LEVELS && (double)value >= 4.0D ? optionName + Lang.get("of.general.max") : (option == GameSettings.Options.SENSITIVITY ? (normalisedValue == 0.0F ? optionName + I18n.format("options.sensitivity.min", new Object[0]) : (normalisedValue == 1.0F ? optionName + I18n.format("options.sensitivity.max", new Object[0]) : optionName + (int)(normalisedValue * 200.0F) + "%")) : (option == GameSettings.Options.FOV ? (value == 70.0F ? optionName + I18n.format("options.fov.min", new Object[0]) : (value == 110.0F ? optionName + I18n.format("options.fov.max", new Object[0]) : optionName + (int)value)) : (option == GameSettings.Options.FRAMERATE_LIMIT ? (value == option.valueMax ? optionName + I18n.format("options.framerateLimit.max", new Object[0]) : optionName + (int)value + " fps") : (option == GameSettings.Options.RENDER_CLOUDS ? (value == option.valueMin ? optionName + I18n.format("options.cloudHeight.min", new Object[0]) : optionName + ((int)value + 128)) : (option == GameSettings.Options.GAMMA ? (normalisedValue == 0.0F ? optionName + I18n.format("options.gamma.min", new Object[0]) : (normalisedValue == 1.0F ? optionName + I18n.format("options.gamma.max", new Object[0]) : optionName + "+" + (int)(value * 100.0F) + "%")) : (option == GameSettings.Options.SATURATION ? optionName + (int)(normalisedValue * 400.0F) + "%" : (option == GameSettings.Options.CHAT_OPACITY ? optionName + (int)(normalisedValue * 90.0F + 10.0F) + "%" : (option == GameSettings.Options.CHAT_HEIGHT_UNFOCUSED ? optionName + GuiNewChat.calculateChatboxHeight(normalisedValue) + "px" : (option == GameSettings.Options.CHAT_HEIGHT_FOCUSED ? optionName + GuiNewChat.calculateChatboxHeight(normalisedValue) + "px" : (option == GameSettings.Options.CHAT_WIDTH ? optionName + GuiNewChat.calculateChatboxWidth(normalisedValue) + "px" : (option == GameSettings.Options.RENDER_DISTANCE ? optionName + (int)value + " chunks" : (option == GameSettings.Options.MIPMAP_LEVELS ? (value == 0.0F ? optionName + I18n.format("options.off", new Object[0]) : optionName + (int)value) : (option == GameSettings.Options.STREAM_FPS ? optionName + TwitchStream.formatStreamFps(normalisedValue) + " fps" : (option == GameSettings.Options.STREAM_KBPS ? optionName + TwitchStream.formatStreamKbps(normalisedValue) + " Kbps" : (option == GameSettings.Options.STREAM_BYTES_PER_PIXEL ? optionName + String.format("%.3f bpp", new Object[] {Float.valueOf(TwitchStream.formatStreamBps(normalisedValue))}): (normalisedValue == 0.0F ? optionName + I18n.format("options.off", new Object[0]) : optionName + (int)(normalisedValue * 100.0F) + "%"))))))))))))))));
            }
            else if (option.getEnumBoolean())
            {
                boolean flag = this.getOptionOrdinalValue(option);
                return flag ? optionName + I18n.format("options.on", new Object[0]) : optionName + I18n.format("options.off", new Object[0]);
            }
            else if (option == GameSettings.Options.GUI_SCALE)
            {
                return this.guiScale >= GUISCALES.length ? optionName + this.guiScale + "x" : optionName + getTranslation(GUISCALES, this.guiScale);
            }
            else if (option == GameSettings.Options.CHAT_VISIBILITY)
            {
                return optionName + I18n.format(this.chatVisibility.getResourceKey(), new Object[0]);
            }
            else if (option == GameSettings.Options.PARTICLES)
            {
                return optionName + getTranslation(PARTICLES, this.particleSetting);
            }
            else if (option == GameSettings.Options.AMBIENT_OCCLUSION)
            {
                return optionName + getTranslation(AMBIENT_OCCLUSIONS, this.ambientOcclusion);
            }
            else if (option == GameSettings.Options.STREAM_COMPRESSION)
            {
                return optionName + getTranslation(STREAM_COMPRESSIONS, this.streamCompression);
            }
            else if (option == GameSettings.Options.STREAM_CHAT_ENABLED)
            {
                return optionName + getTranslation(STREAM_CHAT_MODES, this.streamChatEnabled);
            }
            else if (option == GameSettings.Options.STREAM_CHAT_USER_FILTER)
            {
                return optionName + getTranslation(STREAM_CHAT_FILTER_MODES, this.streamChatUserFilter);
            }
            else if (option == GameSettings.Options.STREAM_MIC_TOGGLE_BEHAVIOR)
            {
                return optionName + getTranslation(STREAM_MIC_MODES, this.streamMicToggleBehavior);
            }
            else if (option == GameSettings.Options.RENDER_CLOUDS)
            {
                return optionName + getTranslation(field_181149_aW, this.clouds);
            }
            else if (option == GameSettings.Options.GRAPHICS)
            {
                if (this.fancyGraphics)
                {
                    return optionName + I18n.format("options.graphics.fancy", new Object[0]);
                }
                else
                {
                    String s2 = "options.graphics.fast";
                    return optionName + I18n.format("options.graphics.fast", new Object[0]);
                }
            }
            else
            {
    ...
    private void setOptionValueOF(GameSettings.Options option, int p_setOptionValueOF_2_)
    {
    ...
    public void loadOfOptions()
    {
        try
        {
            ...
            while ((currentLine = bufferedreader.readLine()) != null)
            {
                try
                {
                    String[] astring = currentLine.split(":");
                ...
                {
                    Config.dbg("Skipping bad option: " + currentLine);
                    exception.printStackTrace();
                }
            }
    ...
    public static enum Options
    {
        ...
        GAMMA("options.gamma", true, false, 0.0F, 10.0F, 1.0F),
        ...
        private Options(String optionName, boolean isFloat, boolean isBoolean)
        {
            this(optionName, isFloat, isBoolean, 0.0F, 1.0F, 0.0F);
        }

        private Options(String optionName, boolean isFloat, boolean isBoolean, float min, float max, float step)
        {
            this.enumString = optionName;
